...
const Chatcontainer = (handleChatIconClick ,userID ,followingId) => {
    const [userchat, setUserchat] = useState([]);
    const fetchuserchat = async () => {
        try {
          const response = await axios.get(`http://localhost:3000/api/userchat/${userID}/${followingId}`);
          console.log(response);
          setUserchat(response.data); // Assuming the API returns { isInWishlist: true/false }
        } catch (error) {
          console.error('Error follouser userchat status:', error);
        }
      };
